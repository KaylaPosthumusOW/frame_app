...
class _FrameButtonState extends State<FrameButton> {
  ...
  Widget _buildLabel(BuildContext context) {
    return Text(
      ...
      style: Theme.of(context).textTheme.labelLarge?.copyWith(color: widget.textColor ?? Colors.white),
    );
  }
  ...
  ButtonStyle _getButtonStyle(bool isDisabled) {
    switch (widget.type) {
      case ButtonType.primary:
        return FilledButton.styleFrom(backgroundColor: isDisabled ? AppColors.framePurple.withValues(alpha: 0.4) : widget.buttonColor ?? AppColors.framePurple, foregroundColor: AppColors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)));
      case ButtonType.outline:
        return OutlinedButton.styleFrom(backgroundColor: AppColors.white, side: BorderSide(color: isDisabled ? AppColors.framePurple : widget.outlineColor ?? AppColors.framePurple), foregroundColor: isDisabled ? AppColors.framePurple : AppColors.framePurple, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)));
    }
  }
